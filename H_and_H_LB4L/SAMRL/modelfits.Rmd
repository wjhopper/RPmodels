---
title: SAM-RL Model Fits to LB4L Cued Recall Data
author: "Will Hopper"
...
```{r subs_imm, results='asis'}
  for (i in 1:length(subplots_imm))  {
    ...
    print(xtable(data.frame(sub_params_imm[i,] , LL = sub_chi_imm[i]),
...
for (i in 1:length(subplots_del))  {
  ...
  print(xtable(data.frame(sub_params_del[i,] , LL = sub_chi_del[i]),
               digits=c(1,4,4,4,4,4,3),
               caption = paste("Best Parameters")),
      type = "html", include.rownames=FALSE, caption.placement="top")    
  '<br>'
...
<<avgs>>
  ...
  df <- data.frame(rbind(sf_par_means_imm,sf_par_means_del),LL= c(sf_chi_means_imm, sf_chi_means_del))
