...
if (!file.exists('aggfits.csv') || !file.exists('subfits.csv')) {
  Matlab$startServer()
  matlab <- Matlab()
  isOpen <- open(matlab)
#   evaluate(matlab, "[aggchi, aggpars, aggfits, subfits, sub_ll,sub_params] = SAMRL('S1',.035,'S2',10,'R',1.5,'R_cor',7,'O1',1.2,'O2',2,'k',15,'p',1, 'fix_params', {'rho','nSubs','k','p'}, 'free_params',{'S1','S2','R','R_cor','O1','O2'}, 'showfigs','Off');")  
  evaluate(matlab, "[aggchi, aggpars, aggfits, subfits, sub_ll,sub_params] = SAMRL('S1',.15,'S2',.4,'R',3,'R_cor',6,'O1',1.2,'O2',2,'k',10,'p',1, 'fix_params', {'rho','nSubs','k','p'},'free_params',{'S1','S2','R','R_cor','O1','O2'}, 'showfigs','Off');")
  ...
  sub_chi <- getVariable(matlab, "sub_ll")
  sub_params <-  getVariable(matlab, "sub_params")
  sub_params<-as.data.frame(sub_params[[1]])
  colnames(sub_params) <- c("O","R","Rcor","S1","S2")
  save(aggchi,aggparams,sub_chi, sub_params,file = "modelingresults.RData")
  close(matlab)
} else {  
  load("modelingresults.RData")
}
...
sub_chi_imm <- sub_chi[[1]][b]
sub_chi_del <- sub_chi[[1]][a]
...
for (j in c('immediate','delay')) {
    ...
    for ( i in unique(subfits$subject[subfits$group==j])) { #unique(subfits$subject[subfits$group==j]))
      ...
      tmp_plot1 <- ggplot(data=df1,aes(x=timepoint,y=pc, shape = variable,color = factor(chain),group=interaction(chain,variable))) +
        geom_point(size =3.5) +
        geom_line() + 
        ...
        ggtitle(paste('Subject',i, 'Final Test Accuracy'))
      ...
      tmp_plot2 <- ggplot(mapping= aes(x=other_type,y=pc,fill=variable)) +
        ...
        mytheme +
        ggtitle(paste('Subject',i, 'Conditional Accuracy'))
      
      if (j =='immediate') {
        subplots_imm[[k]] <- list(tmp_plot1,tmp_plot2)
      } else { 
        subplots_del[[k]] <- list(tmp_plot1,tmp_plot2)
      }
      k=k+1
    }
  }
...
sf_chi_means_imm <- mean(sub_chi_imm)
sf_chi_means_del <- mean(sub_chi_del)
