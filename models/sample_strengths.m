function [ S ] = sample_strengths(mu,rho, interference, nSubjects, nItems)
if nargin < 5
...
means=zeros(nSubjects,nItems,length(mu),length(interference)); %initalize

for i = 1:length(mu) % fill in array pages with the mean values
    means(:,:,i,:)=mu(i);
end

means(:,end+1,:,:)= reshape(repmat(interference,length(mu)*nSubjects,1),nSubjects,1,length(mu),length(interference)); % add the extra-list interference
