function [rec] = recall(S,R,k,one_shot)
...
%% Practice Test 
S_tmp=S(:,:,1,:); % grab sampling strengths from before the test practice round occurs
S_tmp=S_tmp./repmat(sum(S_tmp,2),[1,size(S_tmp,2),1,1]);
R_tmp=R(:,:,1,:);% grab recovery strengths from before the test practice round occurs
...
S_success=S(:,:,2,:); % parse out the highest sampling strengths (e.g. after final practice attempt)
S_fail=S(:,:,1,:); % parse out the sampling strengths form the next highest (i.e. not correct on test practice)
...
R_success=R(:,:,2,:);   % parse out the highest sampling strengths (e.g. after final practice attempt)
R_fail=R(:,:,1,:);    % parse out the sampling strengths form the next highest (i.e. not correct on test practice)
...
%     R_tmp(no_recovery(:,1:end-1,1,1))=0;
    R_tmp(no_recovery(:,1:end-1,one_shot,1))=0;
end
