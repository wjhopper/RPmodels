...
RTdis <- function(RT = NULL, recalled = NULL, Time= NULL) {
...
#   init_thresh <- as.matrix(read.csv('theta.csv'))
  ...
  prac$dist <- RTdis(prac$RT,prac$Acc, p['Time']) 
  ...
  restudy$dist <- RTdis(restudy$RT,restudy$Acc,p['Time']) 
  ...
  test$dist <- RTdis(test$RT,test$Acc,p['Time']) 
