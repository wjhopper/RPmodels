...
if strcmpi(fit,'fit') 
    ...
    if strcmpi(plotting,'iter')    
        [fitted_params, chisquare, ~ , info]=fminsearch(@(x) fit_SAM_RL(x,data,design,fix_params,free_params(:,1),one_shot,plotting,fit),[free_params{:,2}],fmin_opts) %#ok<NASGU,NOPRT,*ASGLU>
    elseif strcmpi(plotting,'final')
        [fitted_params, chisquare, ~ , info]=fminsearch(@(x) fit_SAM_RL(x,data,design,fix_params,free_params(:,1),one_shot,'off',fit),[free_params{:,2}],fmin_opts) %#ok<NASGU,NOPRT,*ASGLU>
        fit_SAM_RL(fitted_params,data,design,fix_params,free_params(:,1),one_shot,plotting,fit); %#ok<NASGU>
        history([],[],'done','final');        
    else
        [fitted_params, chisquare, ~ , info]=fminsearch(@(x) fit_SAM_RL(x,data,design,fix_params,free_params(:,1),one_shot,'off',fit),[free_params{:,2}],fmin_opts) %#ok<NASGU,NOPRT,*ASGLU>
    end    
elseif strcmp(fit,'check')
    chisquare=fit_SAM_RL([free_params{:,2}],data,design,fix_params,free_params(:,1),one_shot,plotting,fit) %#ok<NASGU,NOPRT>
end
